...
(function nashornEventLoopMain(context) {
  ...
  var ConcurrentLinkedDeque = Java.type('java.util.concurrent.ConcurrentLinkedDeque');
...
//   console.log('main javasript thread ' + Thread.currentThread().getName());
  ...
  function resetEventLoop() {
    globalTimerId = new AtomicInteger(1);
    timerMap = new ConcurrentHashMap();
    eventLoop = new ConcurrentLinkedDeque();
  }
...
//             ' in thread ' + Thread.currentThread().getName());
  ...
  var setTimeout = function (fn, millis /* [, args...] */) {
    ...
    var timerId = globalTimerId.getAndIncrement();
